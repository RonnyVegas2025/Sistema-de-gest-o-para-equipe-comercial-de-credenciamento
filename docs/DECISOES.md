...
| A-009 | Varredura de comentários que afirmam invariante sem teste | **Gatilho: a próxima vez que um comentário for encontrado mentindo, ou a revisão de fechamento da Sprint 2** — o que vier antes. Procurar por afirmações de propriedade (estável, imutável, idempotente, nunca nulo, sempre ordenado) e, para cada uma, ou apontar o teste que a sustenta ou removê-la. Um comentário falso não tem quem o reprove: sobrevive à refatoração e é lido como fato |
| A-008 | Seletor com busca no lugar de campo de identificador | **Na auditoria de touch da Sprint 6, não como item solto.** A etapa 5c entregou empresa demandante e consultor como UUID digitado — funciona, e não é usável em tablet no campo. Mesma família da auditoria de 44 px; resolver junto evita criar o componente duas vezes |
